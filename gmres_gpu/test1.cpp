...
void gmres(double *A, double *D, double *x, double *b, int N, int max_restart, int max_iter, double tol)
{
	...
	{
		fastpoisson(b, M_b, N);
		#pragma acc host_data use_device(b, M_b, r)
		{
			cublasHandle_t h;
			cublasCreate(&h);
			cublasDnrm2(h, N2, M_b, 1, normb);
			cublasDcopy(h, N2, M_b, 1, r, 1);
			cublasDestroy(h);
		}
		*beta = *normb;
	}
...
*/
				#pragma acc host_data use_device(Q, H, q, w)
				{
					cublasHandle_t h;
					cublasCreate(&h);
					for (k=0; k<=i; k++)
					{
						#pragma acc parallel loop independent
						for (j=0; j<N2; j++)	q[j] = Q[N2*k+j];
						cublasDdot(h, N2, w, 1, q, 1, nrm_temp);
