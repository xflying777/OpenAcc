...
int main()
{
	...
	t1 = clock();
	#pragma acc data copyin(a[0:n*n], b[0:n*n]) copyout(c_gpu_oacc[0:n*n])
	{
...
/*	printf(" matrix c_gpu_oacc\n ");
        for (i=0; i<n; i++)
        {
                for (j=0; j<n; j++)     printf(" %f ", c_gpu_oacc[i*n+j]);
                printf("\n");
        }
*/
	t1 = clock();
...
/*	printf(" matrix cublas = \n");
	for (i=0; i<n; i++)
	{
		for (j=0; j<n; j++)	printf(" %f ", c_gpu_cublas3[i*n+j]);
		printf("\n");
	}
*/
	int nfailures = 0;
